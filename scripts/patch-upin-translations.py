...
Also adds:
- comingSoon: string to the type
- comingSoon: "Coming Soon" (translated) to each language's projects section

Run: python3 /home/z/my-project/scripts/patch-upin-translations.py
"""
import re
...
# Find: `    confidential: string;`
# Add after: `    comingSoon: string;`
...
# Each language has: `      confidential: "...",`
# Add after: `      comingSoon: "...",`
...
coming_soon_translations = {
    ...
    '\u6a5f\u5bc6': '\u8fd1\u65e5\u516c\u958b',  # ja (機密 → 近日公開)
    '\u0633\u0631\u064a': '\u0642\u0631\u064a\u0628\u0627\u064b',  # ar (سري → قريباً)
    '\u673a\u5bc6': '\u5373\u5c06\u63a8\u51fa',  # zh (机密 → 即将推出)
}

for conf_trans, coming_trans in coming_soon_translations.items():
...
# Position 3 = right before the "Baca" entry (which is currently index 2)

# The project data for each language
projects_data = {
    'en': {
    ...
    'yo': {
        ...
        'description': 'Aplikasyon w\u1eb9\u0301\u1eb9\u0301b\u00f9 \u00e0k\u1ecd\u0301j\u1ecd\u0301p\u1ecd\u0300 logo t\u00ed \u00f3 n\u00ed \u00ecr\u00eds\u00ed al\u1eb9\u0301\u1eb9\u0301k\u00f9\u00fa t\u00ed \u00f3 \u0144 k\u00f3j\u1ecd\u0301 \u00e0w\u1ecd\u0300n \u00e0m\u00ec \u1ecd\u0301k\u1ecd\u0301 74 k\u00e1r\u00edak\u00e1 \u00e0w\u1ecd\u0300n \u00f2w\u00f2 13 \u00e0ti \u00e0w\u1ecd\u0300n \u00ecr\u00eds\u00ed \u00e0y\u00e0w\u00f2 8. A \u1e63\u1eb9\u0301 \u00e0k\u1ecd\u0301s\u00edl\u1eb9\u0301 r\u1eb9\u0300 p\u1eb9\u0300l\u00f9 Next.js 16 \u00e0ti TypeScript, \u00f3 n\u00ed \u00ecd\u00e0nw\u00f2 masonry grid, \u00ecy\u00edpad\u00e0 l\u00e1s\u00eck\u00f2 gidi, w\u00e1d\u00ec\u00ed l\u00e1\u00ec n\u00ed \u00ecd\u00e0\u00e0m\u00f9\u0300, lightbox t\u00ed \u00f3 gbo\u00f2 l\u00e9\u00e8k\u00fan p\u1eb9\u0300l\u00f9 \u00ect\u1ecd\u0301nis\u1ecd\u0301n\u00e0 keyboard, \u00e0ti \u00ecy\u00edpad\u00e0 \u00ecr\u00eds\u00ed al\u1eb9\u0301\u1eb9\u0301k\u00f9\u00fa/\u00ecm\u1ecd\u0301l\u1eb9\u0300. K\u1ecd\u0300\u1ecd\u0300kan logo w\u00e0 n\u00ed \u00ecm\u00fad\u00e0r\u00e0 g\u1eb9\u0301g\u1eb9\u0301b\u00edi \u00e0w\u1ecd\u0300r\u00e1n 1024\u00d71024. W\u1ecd\u0300n y\u00ec\u00ed \u00e1 b\u1ecd\u0301 l\u00e0\u00ecp\u1eb9\u0301.',
...
# Baca entry line numbers per language (from grep earlier)
baca_lines = {
    'en': 415,
    'ms': 880,
    'ha': 1345,
...
# We'll insert BEFORE the Baca entry in each language
# The insertion point is the line with `        {` that opens the Baca block
# which is 2 lines before the `name: "Baca"` line

lines = content.split('\n')

# Process from bottom to top so line numbers don't shift
for lang in ['zh', 'ar', 'ja', 'yo', 'ha', 'ms', 'en']:
    baca_line = baca_lines[lang]  # 1-indexed
    # The `        {` line is 2 lines before `name: "Baca"`
    # So it's at baca_line - 2 (1-indexed) = baca_line - 3 (0-indexed)
    insert_idx = baca_line - 3  # convert to 0-indexed and go back 2 lines

    p = projects_data[lang]
    ...
    # Insert before the Baca block opening
...
print(f'  - Inserted "Your Studio" project at position 3 in all 7 languages')
